              ...
              <div class="card-category bg-white shadow-xs">
                <div class="card-body d-flex bg-light align-items-center ">
                  <div class="bg-orange-100 text-orange-500 rounded-circle d-flex justify-content-center align-items-center  me-3">
                    <input type="checkbox"  {{ $userCategories->contains($category) ? 'checked' : '' }}  class="form-check-input text-orange-500 border-orange-400 " id="category-{{ $category->id }}" name="categories[]" value="{{ $category->id }}" wire:model.defer="categories.{{ $category->id }}"  />
                  </div>
                  <label wire:key="{{ $category->id }}" class="text-green-100" id="category" name="category" for="category-{{ $category->id }}">{{ strip_tags(htmlspecialchars_decode($category->name)) }}</label>
                </div>
              </div>
            </div>
            @endforeach
    ...
    name="regions[]"
    class="form-check-input text-orange-500 border-orange-400" id="region-{{ $region->id }}" name="regions[]" value="{{ $region->id }}" wire:model.defer="regions.{{ $region->id }}"
    :value="region.id">
                  </div>
          ...
          <div class="d-flex justify-content-between mt-3">
            <button type="button" class="prev-step btn btn-primary">Previous</button>
            <input type="submit"wire:loading.remove value="Update" class="update btn btn-primary">
          </div>
        </div>
      </form>
    </div>
